...
-- | For executing abstract syntax list
module Brainhack.Evaluator
  ( VMMemory
  , VMMemoryPointer
...
data BFVirtualMachine = BFVirtualMachine
  { vmMemory         :: VMMemory  -- ^ The state of program result
  , vmMemoryPointer  :: VMMemoryPointer  -- ^ The running machine's memory pointer
  , bfProgramPointer :: BFProgramPointer
  , loopBeginPointerStack :: [BFProgramPointer]-- ^ The VMMemoryPointer is pushed to here when LoopBegin was evaluated
  }
